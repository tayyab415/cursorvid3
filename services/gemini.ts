...
export const generateVideo = async (
    ...
    try {
        ...
        // Note: The Veo SDK types might differ slightly in preview. 
        // For Veo 3.1 Preview, if we have generic reference images (not start/end specific)
        // we map them to the config.referenceImages payload if supported by the model type wrapper.
        // However, the standard `generateVideos` call usually separates `image` (start) and `lastFrame` (end).
        // `referenceImages` is distinct.
        if (referenceImages && referenceImages.length > 0) {
             const refs = referenceImages.map(img => ({
                 ...
                 referenceType: 'ASSET' // Assuming 'ASSET' is the enum string for style/asset reference
             }));
