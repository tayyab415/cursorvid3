...
export const Workspace: React.FC<WorkspaceProps> = ({ 
  ...
  return (
    <div 
      className={`bg-neutral-900 flex flex-col border-r border-neutral-800 transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1.0)] overflow-hidden relative ${className}`}
      style={{ width: isOpen ? '320px' : '0px', minWidth: isOpen ? '320px' : '0px' }}
    >
      <div className="flex flex-col h-full min-w-[320px]"> {/* Inner container fixed width to prevent content squashing during transition */}
        ...
        <div className="p-4 pb-2">
            <div 
            className={`h-24 border-2 border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all group ${isDraggingOver ? 'border-blue-500 bg-blue-500/10' : 'border-neutral-800 hover:border-neutral-600 hover:bg-neutral-800/50'} ${isPickingMode ? 'opacity-50 pointer-events-none' : ''}`}
            onClick={!isPickingMode ? onAddMedia : undefined}
            onDragOver={handleDragOver}
            onDragLeave={handleDragLeave}
            onDrop={handleDrop}
            >
            <div className="p-2 rounded-full bg-neutral-800 group-hover:bg-neutral-700 transition-colors mb-2">
                <Plus className="w-4 h-4 text-neutral-400 group-hover:text-blue-400" />
            </div>
            <span className="text-xs font-medium text-neutral-500 group-hover:text-neutral-300">Add Media / Generate</span>
            <span className="text-[9px] text-neutral-600 mt-1">or drag files here</span>
            </div>
        </div>
